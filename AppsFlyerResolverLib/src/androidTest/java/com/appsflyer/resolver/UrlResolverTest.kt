...
class UrlResolverTest {
    ...
    fun  testResolveMILink(){
        runBlocking{
            val res =  AFHttp().resolveDeepLinkValue( "https://mi.gap.com/p/cp/d46c31d50872e4b3/c?mi_u=51635872&EV=GPUSCPATHTSTPC4048593PERST1_WMNNLPSPRP04162021&DI=51635872&CD=GPNC_GPR&cvosrc=email.exacttarget.GPUS04162021&EV_Segment1=GPUSGWMET&url=https%3A%2F%2Fmi.gap.com%2Fp%2Frp%2F6d2aa0caedfe8322%2Furl&mi_u=%%email_key%%&EV=%%EV_value%%&DI=%%DI_value%%&CD=%%CD_value%%", 5)
    ...
    fun  testNullURL(){
        runBlocking{
            val res =  AFHttp().resolveDeepLinkValue(null, 5)
    ...
    fun  testNotaURL(){
        runBlocking{
            val res =  AFHttp().resolveDeepLinkValue("abcd", 5)
    ...
    fun  testEmptyString(){
        runBlocking{
            val res =  AFHttp().resolveDeepLinkValue("", 5)
            assertEquals(res , "")
        }
